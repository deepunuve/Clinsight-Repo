...
import { transferFilesFromGDrive } from '../../api/source';
...
const Gdrive = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!token.trim()) {
            setError('Google drive api key cannot be empty');
            return;
        }
        if (!folderId.trim()) {
            setFolderError('drive link cannot be empty');
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            const response = await transferFilesFromGDrive(token, folderId);
            if (response)
                setResult(response);
            else
