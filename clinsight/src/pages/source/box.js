...
import { transferFilesFromBox } from '../../api/source';
...
const Box = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!folderId.trim()) {
            setFolderError('Folder Identifier cannot be empty');
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            const response = await transferFilesFromBox(token, folderId);
            if (response)
                setResult(response);
            else
                            ...
                            <Card.Body>
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="mb-3">
                                        <input
                                            type="text"
                                            placeholder="Enter box api token"
                                            value={token}
                                    ...
                                    <div className="mb-3">
                                        <input
                                            type="text"
                                            placeholder="Enter box folder identifier"
                                            value={folderId}
